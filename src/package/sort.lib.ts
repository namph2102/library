function quickSortArray<T>(list: T[] = [], left: number, right: number) {
  if (list.length <= 1) return list;
  let pilot = list[Math.floor((right + left) / 2)];
  let l = left,
    r = right;
  do {
    while (list[l] < pilot) l++;
    while (list[r] > pilot) r--;
    if (l <= r) {
      const tam = list[l];
      list[l] = list[r];
      list[r] = tam;
...
function quickSortObject<T>(
  ...
  let l = left,
    r = right;
  do {
    while (list[l][key] < pilot) l++;
    while (list[r][key] > pilot) r--;
    if (l <= r) {
      const tam = list[l];
      list[l] = list[r];
      list[r] = tam;
 ...
 */
export default function handleSort<T>(list: T[], key?: keyof T) {
  ...
  if (typeof list[0] == "object") {
    if (key) {
      ...
      if (list[0]) {
        const findKey: any = Object.keys(list[0])[0];
