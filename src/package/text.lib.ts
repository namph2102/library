...
const timeSetTing: Record<string, string> = {
  months: "tháng",
  month: "tháng",
  years: "năm",
  year: "năm",
...
export function timeFormatComment(
  timeStart: string = "2023-08-15T03:28:02.734+00:00",
  timeEnd?: string
) {
  // trả về thời gian
...
export const CheckTypeImage = (filePath: string) => {
...
export class TextHandler {
  static copyText(textToCopy: string) {
...
export class Validator {
   ...
   * @returns boolean
   */
  static isMinlength(text: string, minlength: number) {
   ...
   * @returns boolean
   */
  static isMaxLength(text: string, maxlength: number) {
...
export function cn(...classnames: clsx.ClassValue[]) {
  return twMerge(clsx.clsx(classnames));
}
